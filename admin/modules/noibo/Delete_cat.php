...
$result = $db->sql_query("SELECT catid, parent FROM {$prefix}_noibo_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC");
...
$query = "SELECT time, images FROM ".$prefix."_noibo WHERE $condition";
$resultimg = $db->sql_query($query);
while(list($time, $images) = $db->sql_fetchrow($resultimg)) {
	$get_path = get_path($time);
	$path_upload_img = "$path_upload/noibo/$get_path";
	@unlink("../$path_upload_img/$images");
	@unlink("../$path_upload_img/thumb_".$images."");
	@unlink("../$path_upload_img/bl_".$images."");
}

$query = "DELETE FROM {$prefix}_noibo WHERE $condition";
$db->sql_query($query);
$query = "DELETE FROM {$prefix}_noibo_cat WHERE $condition";
$db->sql_query($query);

truncate_table("noibo_cat");
fixweight_cat();
