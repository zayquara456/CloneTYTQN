...
if (isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		$db->sql_query("UPDATE ".$prefix."_video_cat SET title='$title',permalink='$permalink',guid='$guid', seo_title='$title_seo', seo_description='$description_seo', seo_keyword='$keyword_seo', images='$images1', active='$active', onhome='$onhome', homelinks='$homelinks', parent=$parent, cat_type='$cat_type' WHERE catid='$catid'");
		fixweight_cat();
...
$result = $db->sql_query("SELECT title, seo_title, seo_description, seo_keyword, images, active, onhome, homelinks, parent FROM ".$prefix."_video_cat WHERE catid=$catid AND alanguage='$currentlang'");
...
//hien thi tat ca cac cap danh muc tin tuc
	$resultcat = $db->sql_query("SELECT catid, title FROM ".$prefix."_video_cat WHERE parent='0' AND catid!='$catid' AND alanguage='$currentlang' ORDER BY weight");
