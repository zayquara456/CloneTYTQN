...
$load_hf = 1;
$resultinfo = $db->sql_query("SELECT catid, title FROM {$prefix}_video_cat WHERE catid='$catid'");
if ($db->sql_numrows($resultinfo) > 0) {
	list($catid, $title) = $db->sql_fetchrow($resultinfo);
}
$result = $db->sql_query("SELECT catid, parent FROM {$prefix}_video_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC");
