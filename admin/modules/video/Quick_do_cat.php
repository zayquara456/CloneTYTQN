...
if ($fc == 2) {
	for($i =0; $i < sizeof($catid); $i ++) {
		$db->sql_query("UPDATE ".$prefix."_video_cat SET active='0' WHERE catid='$catid[$i]'");
	}	
}	

if ($fc == 3) {
	for($i =0; $i < sizeof($catid); $i ++) {
		$db->sql_query("UPDATE ".$prefix."_video_cat SET active='1' WHERE catid='$catid[$i]'");
	}	
}
...
if ($fc == 4) {
	foreach ($poz as $catidx => $weight) {
		...
		$db->sql_query("UPDATE ".$prefix."_video_cat SET weight='$weight' WHERE catid='$catidx'");
	}
	fixweight_cat();
}		
