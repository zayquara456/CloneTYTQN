...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		list ($xcatid) = $db->sql_fetchrow($db->sql_query("SELECT max(catid) AS xid FROM ".$prefix."_video_cat"));
		if ($xcatid == "-1") { $catid = 1; } else { $catid = $xcatid + 1; }
		$weight = WeightMax("_video_cat", $parentid, '', 'parent');
		$ckresult = $db->sql_query("SELECT permalink FROM ".$prefix."_video_cat WHERE permalink='$permalink'");
		if($db->sql_numrows($ckresult) > 0) {
			$permalink=$permalink.'-'.$catid;
		}
		$db->sql_query("INSERT INTO ".$prefix."_video_cat (catid, title, permalink, seo_title, seo_description, seo_keyword, images, alanguage, active, weight, onhome, homelinks, parent, counts, startid, cat_type) VALUES (NULL, '$title', '$permalink', '$title_seo', '$description_seo', '$keyword_seo', '$images', '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0, '$cat_type')");
		
		fixweight_cat();
		updateadmlog($admin_ar[0], _MODTITLE, 'Thêm chuyên ', 'Thêm chuyên mục '.$title);
		//update guid
		$guid="index.php?f=".$adm_modname."&do=categories&id=$catid";
		$query = "UPDATE ".$prefix."_video_cat SET guid='$guid' WHERE catid='$catid'";
		$db->sql_query($query);
...
$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_video_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
...
$resultcat = $db->sql_query("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_video_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC ");
if($db->sql_numrows($resultcat) > 0) {
