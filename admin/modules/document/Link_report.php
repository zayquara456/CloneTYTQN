...
echo "<div id=\"pagecontent\">";
	...
	while(list($id, $docid, $time, $name, $email, $url, $url_replace, $title, $content, $status) = $db->sql_fetchrow($result)) {
		$css ="row1";
			switch($status) {
				case 0: $status = "<a href=\"?f=".$adm_modname."&do=views_link&id=$id\" info=\"chưa xử lý\"><i class=\"fa fa-minus-circle fa-lg\"></i></a>"; break;
				case 1: $status = "<a href=\"?f=".$adm_modname."&do=views_link&id=$id\" info=\"đang xử lý\"><i class=\"fa fa-ellipsis-h\"></i></a>"; break;
				case 2: $status = "<a href=\"?f=".$adm_modname."&do=views_link&id=$id\" info=\"đã xử lý\"><i class=\"fa fa-check-circle fa-lg fa-green\"></i></a>"; break;
			}
