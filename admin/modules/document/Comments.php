...
$newsArr = array();
$newstitle = $db->sql_query("SELECT id, title FROM {$prefix}_document");
while (list($nid, $ntitle) = $db->sql_fetchrow()) $newsArr[$nid] = $ntitle;
$total = $db->sql_numrows($db->sql_query("SELECT id FROM {$prefix}_document_comments WHERE alanguage='$currentlang'"));
$result = $db->sql_query("SELECT id, documentid, title, content, name, email, time, status FROM {$prefix}_document_comments WHERE alanguage='$currentlang' $sortby LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	while(list($id, $documentid, $title, $content, $name, $email, $time, $status) = $db->sql_fetchrow($result)) {
		if (($i % 2) == 1) $css = "row1";
		else $css ="row3";
			switch($status) {
		...
		echo "<td class=\"$css\"><b><a href=\"".RPATH.url_sid("index.php?f=".$adm_modname."&do=detail&id=$documentid")."\" info=\""._VIEW."\" target=\"_blank\">".titlenews_byid($documentid)."</a></b></td>\n";
	...
	echo "<tr><td colspan=\"9\" class=\"row3\"><div class=\"fl\"><select name=\"fc\">";
	...
	echo "</select>&nbsp;<input type=\"submit\" name=\"submit\" value=\""._DOACTION."\"></form></div>";
	if($total > $perpage) {
		echo "<div class=\"fr\">";
