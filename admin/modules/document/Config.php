...
if(isset($_POST['submit'])) {
	...
	$content .= "}\n";
	$content .= "\n";
	$content .= "\$document_home_type = $config0;\n";
	$content .= "\$perpage = $config2;\n";
	$content .= "\$document_ccd = $config3;\n";
	$content .= "\$pic_align = \"$config4\";\n";
	$content .= "\$pic_align_newst = \"$config6\";\n";
	$content .= "\$pic_align_cat = \"$config15\";\n";
	$content .= "\$sizedocument = $config5;\n";
...
for($i = 0; $i < 2; $i ++) {
	$seld ="";
	if($i == $news_home_type) { $seld =" selected"; }
...
for($i = 0; $i < 2; $i ++) {
	$seld ="";
	if($pozar[$i] == $pic_align_newst) { $seld =" selected"; }
	echo "<option value=\"$pozar[$i]\"$seld>$poz_nam[$i]</option>";
}
echo "</select></td>\n";
echo "</tr>\n";
...
echo "<td class=\"row3\"><input type=\"text\" name=\"config5\" value=\"$sizenews\" size=\"10\"> (pixels)</td>\n";
echo "</tr>\n";
