...
if (isset($_POST['submit'])) {
	...
	if(!$err) {
		...
		if ((($_GET['type'] == 'normal') && (!$timed)) || (($_GET['type'] == 'timed') && ($timed))) {
			...
			$db->sql_query("INSERT INTO {$prefix}_news_temp (catid, title, alanguage, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, news_type, images, imgtext,  source, imgshow, othershow, image_highlight, hits, nstart, special, timed) VALUES ($catid, '$title', '$currentlang', '$hometext', '$bodytext', '$title_seo', '$description_seo', '$keyword_seo', '$tag_seo', '$fattach', '$news_type', '$images', '$imgtext',  '$source', $imgshow, '$othershow', $highlight,  0, $startid, $special, FROM_UNIXTIME($postTime))");
		} elseif (($_GET['type'] == 'timed') && (!$timed)) {
			$db->sql_query("DELETE FROM $table WHERE id=$id");
			$db->sql_query("INSERT INTO {$prefix}_news (catid, title, alanguage, time, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, news_type, images, imgtext, source, imgshow, othershow, image_highlight, hits, nstart, special) VALUES ($catid, '$title', '$currentlang', ".time().", '$hometext', '$bodytext', '$title_seo', '$description_seo', '$keyword_seo', '$tag_seo', '$fattach', '$news_type', '$images', '$imgtext', '$source', $imgshow, '$othershow', $highlight, 0, $startid, $special)");
		}
		if (($db->sql_affectedrows() > 0) && (!$timed)) {
			...
			if($startid == 1) {
				...
				$db->sql_query("UPDATE {$prefix}_news SET nstart=0 WHERE id!=$lastInsertId AND catid=$catid");
				$db->sql_query("UPDATE {$prefix}_news_cat SET startid=$lastInsertId WHERE catid=$catid");
			}
		}
