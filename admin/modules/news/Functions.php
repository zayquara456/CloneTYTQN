...
function fixweight_cat() {
    ...
    $result = $db->sql_query("SELECT catid, weight FROM ".$prefix."_news_cat WHERE alanguage='$currentlang' order by weight ASC");
    $weight = 0;
    while($row = $db->sql_fetchrow($result)) {
	    $catid = $row['catid'];
		$weight++;
	    $catid = intval($catid);
		$db->sql_query("UPDATE ".$prefix."_news_cat SET weight='$weight' WHERE catid='$catid'");
    }
}

function fixcount_cat() {
	global $prefix, $db;
	 $result = $db->sql_query("SELECT catid, counts FROM ".$prefix."_news_cat");
	 $i =0;
	 while (list($catid, $counts) = $db->sql_fetchrow($result)) {
	 	$numsnew = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_news WHERE catid=$catid"));
	 	if($counts != $numsnew) {
	 		$db->sql_query("UPDATE ".$prefix."_news_cat SET counts=$numsnew WHERE catid=$catid");
	 	}
	 	$i++;
	 }
}
