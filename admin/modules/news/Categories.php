...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		list ($xcatid) = $db->sql_fetchrow($db->sql_query("SELECT max(catid) AS xid FROM ".$prefix."_news_cat"));
		...
		$db->sql_query("INSERT INTO ".$prefix."_news_cat (catid, title, permalink, alanguage, active, weight, onhome, homelinks, parent, counts, startid, cat_type) VALUES (NULL, '$title', '$permalink', '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0, '$cat_type')");
		
		fixweight_cat();
		updateadmlog($admin_ar[0], _MODTITLE, 'Thêm chuyên mục', 'Thêm chuyên mục '.$title);
		//update guid
		$guid="index.php?f=news&do=categories&id=$catid";
		$query = "UPDATE ".$prefix."_news_cat SET guid='$guid' WHERE catid='$catid'";
		$db->sql_query($query);
...
$resultcat = $db->sql_query("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_news_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC ");
if($db->sql_numrows($resultcat) > 0) {
