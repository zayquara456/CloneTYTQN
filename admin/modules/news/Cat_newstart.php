...
$result_cat = $db->sql_query("SELECT title, startid FROM ".$prefix."_news_cat WHERE catid='$catid'");
...
if($action == "choose") {
	$db->sql_query("UPDATE ".$prefix."_news_cat SET startid='$sid' WHERE catid='$catid'");
	header("Location: ".$adm_modname.".php?do=categories");
	exit;
}	
...
$total = $db->sql_numrows($db->sql_query("SELECT*FROM ".$prefix."_news WHERE catid='$catid' AND alanguage='$currentlang'"));
$result = $db->sql_query("SELECT id, title, time, active, hits FROM ".$prefix."_news WHERE catid='$catid' AND alanguage='$currentlang' $sortby LIMIT $offset, $perpage");
