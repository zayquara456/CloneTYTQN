...
$countf = $db->sql_fetchrow($db->sql_query("SELECT COUNT(*) FROM ".$prefix."_products WHERE alanguage='$currentlang' AND pnews='1'"));
$total = ($countf[0]) ? $countf[0] : 1;
$result = $db->sql_query("SELECT id, title, time, active, hits FROM ".$prefix."_products WHERE pnews='1' AND alanguage='$currentlang' $sortby LIMIT $offset,$perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<tr><td colspan=\"9\" class=\"header\">"._PRD_LIST1."</td></tr>";
	echo "<tr>\n";
	echo "<td class=\"row1sd\" width=\"20\" align=\"center\">".sortBy("modules.php?f=$adm_modname&do=pnews",1)."</td>\n";
	...
	echo "<td class=\"row1sd\" align=\"center\" width=\"100\">"._TIMEUP." ".sortBy("modules.php?f=$adm_modname&do=pnews",3)."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"60\">"._STATUS."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"80\">"._VIEW." ".sortBy("modules.php?f=$adm_modname&do=pnews",5)."</td>\n";
	...
	if($total > $perpage) {
		...
		$pageurl = "modules.php?f=".$adm_modname."&do=pnews&sort=$sort";
	...
	echo "<option value=\"6\">&raquo; "._QUICKDO_PRD3."</option>";
