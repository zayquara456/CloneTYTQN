...
switch($sort) {
	case 1: $sortby ="ORDER BY title ASC"; break;
	case 2: $sortby ="ORDER BY title DESC"; break;
	case 3: $sortby ="ORDER BY time ASC"; break;
	case 4: $sortby ="ORDER BY time DESC"; break;
	case 5: $sortby ="ORDER BY status ASC"; break;
	case 6: $sortby ="ORDER BY status DESC"; break;
...
$result = $db->sql_query("SELECT id, title, source, data, time, status FROM {$prefix}_ngrab_filter $where $sortby LIMIT $offset, $perpage");
...
<form action="" name="frmtool" method="get">
	...
	<input class="button2" type="button" onclick="window.location='modules.php?f=crawler&do=create'" value="<?php echo _CREATE_FILTER?>">&nbsp;&nbsp;
    <input class="button2" type="button" onclick="window.location='modules.php?f=crawler'" value="<?php echo _LIST_FILTER?>">&nbsp;&nbsp;
...
echo "<div id=\"pagecontent\">";
	...
	while(list($id, $title, $source, $data, $time, $status) = $db->sql_fetchrow($result)) {
		...
		if($ajax_active == 1) {
			switch($status) {
				case 1: $status = "<a href=\"?f=".$adm_modname."&do=status&id=$id&stat=0\" title=\""._DEACTIVATE."\" onclick=\" aj_base_status($id,'0','$adm_modname','status_news',mid); return false;\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $status = "<a href=\"?f=".$adm_modname."&do=status&id=$id&stat=1\" title=\""._ACTIVE."\" onclick=\" aj_base_status($id,'1','$adm_modname','status_news',mid); return false;\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		} else {
			switch($status) {
				case 1: $status = "<a href=\"?f=".$adm_modname."&do=status&id=$id&stat=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $status = "<a href=\"?f=".$adm_modname."&do=status&id=$id&stat=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		}
		...
		echo "<td align=\"center\" class=\"$css\"><a class=\"fancybox fancybox.iframe\" href=\"?f=".$adm_modname."&do=view&id=$id\" title=\""._VIEW."\">"._VIEW."</a></td>\n";
		...
		if($ajax_active == 1) {
			echo "<td align=\"center\" width=\"30\" class=\"row3\"><a href=\"?f=".$adm_modname."&do=delete&id=$id\" title=\""._DELETE."\" onclick=\"aj_base_delete($id,'$adm_modname','"._DELETEASK1."','delete_news','');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
