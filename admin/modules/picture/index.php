...
$catArr = array();
$cats = $db->sql_query("SELECT catid, title FROM {$prefix}_picture_cat");
while (list($cid, $ctitle) = $db->sql_fetchrow()) $catArr[$cid] = $ctitle;
$total = $db->sql_numrows($db->sql_query("(SELECT id FROM {$prefix}_picture WHERE alanguage='$currentlang') UNION (SELECT id FROM {$prefix}_picture_temp WHERE alanguage='$currentlang')"));
$result = $db->sql_query("(SELECT id, catid, title, time, active, hits, nstart, 'normal' FROM {$prefix}_picture WHERE alanguage='$currentlang') UNION (SELECT id, catid, title, UNIX_TIMESTAMP(timed) AS time, active, hits, nstart, 'timed' FROM {$prefix}_picture_temp WHERE alanguage='$currentlang') $sortby LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	while(list($id, $catid, $title, $time, $active, $hits, $nstart, $newsType) = $db->sql_fetchrow($result)) {
		...
		if($ajax_active == 1) {
			switch($active) {
				case 1: $active = "<a href=\"?f=".$adm_modname."&do=status_picture&type=$newsType&id=$id&stat=0\" title=\""._DEACTIVATE."\" onclick=\"ajaxinfoget('modules.php?f=news&do=status_picture&type=$newsType&id=$id&stat=0&load_hf=1','ajaxload_container', 'news_main'); return false;\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"?f=".$adm_modname."&do=status_picture&type=$newsType&id=$id&stat=1\" title=\""._ACTIVE."\" onclick=\"ajaxinfoget('modules.php?f=news&do=status_picture&type=$newsType&id=$id&stat=1&load_hf=1','ajaxload_container', 'news_main'); return false;\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
			switch($nstart) {
				case 1: $nstart = "<a href=\"?f=".$adm_modname."&do=news_start&type=$newsType&id=$id&stat=0\" info=\""._NOSTART."\" onclick=\"ajaxinfoget('modules.php?f=news&do=news_start&type=$newsType&id=$id&stat=0','ajaxload_container','news_main'); return false;\"><img border=\"0\" src=\"../images/start.png\"></a>"; break;
				case 0: $nstart = "<a href=\"?f=".$adm_modname."&do=news_start&type=$newsType&id=$id&stat=1\" info=\""._YESSTART."\" onclick=\"ajaxinfoget('modules.php?f=news&do=news_start&type=$newsType&id=$id&stat=1','ajaxload_container','news_main'); return false;\"><img border=\"0\" src=\"../images/starto.png\"></a>"; break;
			}
		} else {
			switch($active) {
				case 1: $active = "<a href=\"?f=".$adm_modname."&do=status_picture&type=$newsType&id=$id&stat=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"?f=".$adm_modname."&do=status_picture&type=$newsType&id=$id&stat=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
			switch($nstart) {
		...
		if ($newsType == 'normal') $titleLink = "<b>$title</b> <a href=\"../".url_sid("index.php?f=".$adm_modname."&do=detail&id=$id&t=".cv2urltitle($title)."")."\" info=\""._GETLINK."\" onclick=\"prompt('"._GETLINK."','".url_sid("index.php?f=$adm_modname&do=detail&id=$id&t=".cv2urltitle($title)."")."'); return false;\"><img border=\"0\" src=\"images/link.png\"></a>";
		...
		echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=edit_picture&type=$newsType&id=$id\" info=\""._EDIT."\"><img border=\"0\" src=\"images/edit.png\"></a></td>\n";
		if($ajax_active != 1) {
			echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=delete_picture&type=$newsType&id=$id\" title=\""._DELETE."\" onclick=\"ajaxinfoget('modules.php?f=news&do=delete_picture&type=$newsType&id=$id&load_hf=1','ajaxload_container', 'news_main'); return false; aj_base_delete($id,'$adm_modname','"._DELETEASK1."','delete_picture','');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
			echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=delete_picture&type=$newsType&id=$id\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK1."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		}
		echo "</tr>\n";
		$i++;
	}
