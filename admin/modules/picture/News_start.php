...
if ($_GET['type'] == 'timed') $table = "{$prefix}_picture_temp";


if($stat == 0) {
	$db->sql_query("UPDATE $table SET nstart=0 WHERE id=$id");
	if ($_GET['type'] == 'normal') $db->sql_query("UPDATE {$prefix}_picture_cat SET startid=0 WHERE catid=$catid AND startid=$id AND alanguage='$currentlang'");
} else if ($stat == 1) {
	...
	$db->sql_query($query);
	if ($_GET['type'] == 'normal') $db->sql_query("UPDATE {$prefix}_picture_cat SET startid=$id WHERE catid=$catid AND alanguage='$currentlang'");
}
