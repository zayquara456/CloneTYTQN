...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query("INSERT INTO ".$prefix."_picture_cat (catid, title, alanguage, active, weight, onhome, homelinks, parent, counts, startid,images) VALUES (NULL, '$title', '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0,'$images')");
		fixweight_cat();
...
$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_picture_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
	...
	foreach ($newArr as $key => $val) {
		...
		if(intval($tempArr['counts']) > 0) {
			...
			if(intval($tempArr['startid']) == 0) {
				$startids = "<a href=\"".$adm_modname.".php?f=".$adm_modname."&do=cat_picturetart&catid={$tempArr['id']}\">"._CHOOSE."</a>";
			} else {
		...
		if ($ajax_active == 1) {
			...
			$title = "<a href=\"?f=$adm_modname&do=quick_title_cat&id={$tempArr['id']}\" info=\""._QUICK_EDIT."\" onclick=\"return show_edit_title({$tempArr['id']},'{$tempArr['title']}','$adm_modname',20,'"._SAVECHANGES."','quick_title_cat');\">{$tempArr['title']}</a> <a href=\"../".url_sid("index.php?f=".$adm_modname."&do=categories&catid={$tempArr['id']}&t=".cv2urltitle($tempArr['title'])."")."\" info=\""._GETLINK."\" onclick=\"prompt('"._GETLINK."','".url_sid("index.php?f=$adm_modname&do=categories&catid={$tempArr['id']}&t=".cv2urltitle($tempArr['title'])."")."'); return false;\"><img border=\"0\" src=\"images/link.png\"></a>";
...
$resultcat = $db->sql_query("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_picture_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC ");
if($db->sql_numrows($resultcat) > 0) {
