...
if($stat == 0) {
	...
	if ($_GET['type'] == 'normal') $db->sql_query("UPDATE {$prefix}_download_cat SET startid=0 WHERE catid=$catid AND startid=$id AND alanguage='$currentlang'");
} else if ($stat == 1) {
	...
	$db->sql_query($query);
	if ($_GET['type'] == 'normal') $db->sql_query("UPDATE {$prefix}_download_cat SET startid=$id WHERE catid=$catid AND alanguage='$currentlang'");
}
