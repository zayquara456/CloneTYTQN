...
$result = $db->sql_query("SELECT id, docid, time, name, email, url, title, content, status FROM {$prefix}_link_report $where $sortby LIMIT $offset, $s_quantity");
...
echo "<div id=\"pagecontent\">";
	...
	while(list($id, $docid, $time, $name, $email, $url, $title, $content, $status) = $db->sql_fetchrow($result)) {
		$css ="row1";
			switch($status) {
				case 1: $status = "<a href=\"?f=".$adm_modname."&do=status_link&id=$id&status=0\" info=\"Chuyển về chế độ đang chờ kiểm tra\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $status = "<a href=\"?f=".$adm_modname."&do=status_link&id=$id&status=1\" info=\"Chuyển về chế độ đã kiểm tra và khắc phục lỗi\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		...
		echo "<td class=\"$css\"><b><a href=\"$url\">$url</a></b></td>\n";
