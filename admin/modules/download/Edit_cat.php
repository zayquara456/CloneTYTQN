...
if (isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		$db->sql_query("UPDATE ".$prefix."_download_cat SET title='$title',permalink='$permalink',guid='$guid' , active='$active', onhome='$onhome', homelinks='$homelinks', parent=$parent, cat_type='$cat_type' WHERE catid='$catid'");
		fixweight_cat();
...
$result = $db->sql_query("SELECT title, active, onhome, homelinks, parent FROM ".$prefix."_download_cat WHERE catid=$catid AND alanguage='$currentlang'");
...
echo "<form method=\"POST\" action=\"modules.php?f=download&do=edit_cat&id=$catid\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder\">\n";
...
//hien thi tat ca cac cap danh muc tin tuc
	$resultcat = $db->sql_query("SELECT catid, title FROM ".$prefix."_download_cat WHERE parent='0' AND catid!='$catid' AND alanguage='$currentlang' ORDER BY weight");
	if($db->sql_numrows($resultcat) > 0) {
		...
		while(list($cat_id, $titlecat) = $db->sql_fetchrow($resultcat)) {
			...
			$listcat .= subcat_notnull($cat_id,"-",$catid, $catid);
		}
		echo $listcat;
		echo "</select></td>\n";
		echo "</tr>\n";
