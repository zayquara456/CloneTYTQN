...
if(empty($_SESSION['linkpage_download']))
	$_SESSION['linkpage_download']="".$_SERVER['QUERY_STRING']."";
...
$catArr = array();
$cats = $db->sql_query("SELECT catid, title FROM {$prefix}_{$adm_modname}_cat");
...
$total = $db->sql_numrows($db->sql_query("SELECT id FROM {$prefix}_{$adm_modname} $where"));
$result = $db->sql_query("SELECT id, catid, title, time, active, hits, nstart, 'normal' FROM {$prefix}_{$adm_modname} $where $sortby LIMIT $offset, $perpage");
...
<form action="" name="frmtool" method="get">
	...
	<?php
	$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_{$adm_modname}_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
...
echo "<div id=\"pagecontent\">";
	...
	while(list($id, $catid, $title, $time, $active, $hits, $nstart, $newsType) = $db->sql_fetchrow($result)) {
		...
		if($ajax_active == 1) {
			echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=delete_news&type=$newsType&id=$id\" title=\""._DELETE."\" onclick=\"aj_base_delete($id,'$adm_modname','"._DELETEASK1."','delete_news','');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
	...
	echo "<div class=\"fr\">";
	if($total > $perpage) {
		$pageurl = "modules.php?f=download&sort=$sort";
