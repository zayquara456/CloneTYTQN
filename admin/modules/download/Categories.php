...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		list ($xcatid) = $db->sql_fetchrow($db->sql_query("SELECT max(catid) AS xid FROM ".$prefix."_download_cat"));
		if ($xcatid == "-1") { $catid = 1; } else { $catid = $xcatid + 1; }
		$weight = WeightMax("download_cat", $parentid, '', 'parent');
		$db->sql_query("INSERT INTO ".$prefix."_download_cat (catid, title, permalink, alanguage, active, weight, onhome, homelinks, parent, counts, startid, cat_type) VALUES (NULL, '$title', '$permalink', '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0, '$cat_type')");
		
		fixweight_cat();
		updateadmlog($admin_ar[0], $adm_modname, _MODTITLE, _CREATE_NEWS_TOPIC);
		//update guid
		$guid="index.php?f=download&do=categories&id=$catid";
		$query = "UPDATE ".$prefix."_download_cat SET guid='$guid' WHERE catid='$catid'";
		$db->sql_query($query);
...
$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_download_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
...
echo "<tr><td colspan=\"2\" align=\"center\" class=\"row1\"><input type=\"submit\" class=\"button2\" id=\"submit\" name=\"submit\" value=\""._ADD."\"></td></tr>";
echo "</table></form>";
echo "</div>";
...
$resultcat = $db->sql_query("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_download_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC ");
if($db->sql_numrows($resultcat) > 0) {
