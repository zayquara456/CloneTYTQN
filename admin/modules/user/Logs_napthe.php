...
<form action="" name="frmtool" method="get">
	...
	<input type="hidden" name="do" value="logs_napthe" />
	...
	<label for="cat"></label>
	<?php
	$resultcat = $db->sql_query("SELECT DISTINCT title FROM {$prefix}_napthe_log ORDER BY title DESC");
...
$result = $db->sql_query("SELECT id, user_id, dateline, area, title, action, status, money_old, money, money_new, ip_add FROM {$prefix}_napthe_log $where LIMIT $s_quantity");
...
if($db->sql_numrows($result) > 0) {
	...
	echo "<td class=\"row1sd\">Hành động</td>\n";
