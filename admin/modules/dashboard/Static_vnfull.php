...
<?php
	...
	echo "<div style=\"text-align:center; font-weight:bold; font-size:16px; padding-bottom:20px\"><h1>THỐNG ĐĂNG TÀI LIỆU WEBSITE THUVIENXAYDUNG.NET</h1><span>Tháng ".date('m',strtotime($ckmonth))." năm ".date('Y',strtotime($ckmonth))."</div>";
	//echo "<table border=\"1\" width=\"100%\" cellspacing=\"4\"  cellpadding=\"4\"  class=\"tableborder\">\n";
	//echo "<tr><td colspan=\"31\" class=\"header\">I - Tổng tài liệu đăng tải</td></tr>";
	//echo "<tr>\n";
	//		echo "<td align=\"center\" class=\"row1sd\" width=\"10px\">STT</td>\n";
	//		echo "<td align=\"center\" class=\"row1sd\">Tổng tài liệu đã đăng</td>\n";
	//		echo "<td align=\"center\" class=\"row1sd\">Tổng số người đăng</td>\n";
	//echo "</tr>\n";
	//echo "<tr>\n";
	...
	echo "<br>";
	echo "<table border=\"1\" width=\"100%\" cellspacing=\"4\"  cellpadding=\"4\" class=\"tableborder\">\n";
	echo "<tr><td colspan=\"31\" class=\"header\">I - Lượt đăng tải thành viên</td></tr>";
	echo "<tr>\n";
			echo "<td align=\"center\" class=\"row1sd\" width=\"10px\">STT</td>\n";
			echo "<td align=\"center\" class=\"row1sd\">Tài khoản</td>\n";
			echo "<td align=\"center\" class=\"row1sd\" width=\"60px\">Tài liệu</td>\n";
	...
	echo "</tr>\n";
		$resultup = $db->sql_query("SELECT u.adacc, u.adname, u.id FROM ".$prefix."_document as d, ".$prefix."_admin as u WHERE d.uadmin=u.id and '$ckmonth'=DATE_FORMAT(FROM_UNIXTIME(d.time),'%Y-%m') AND d.active=1 GROUP BY u.adacc order by u.adacc desc");
		if($db->sql_numrows($resultup) > 0){
			$i=1;
		while(list($fullname, $adname, $id) = $db->sql_fetchrow($resultup)){
	echo "<tr>\n";
			//$thuclinh=$sumprice*40000/100;
			echo "<td align=\"center\" class=\"row1\">$i</td>\n";
			echo "<td align=\"left\" class=\"row1\">".$adname." (".$fullname.")</td>\n";
			echo "<td align=\"center\" class=\"row1\">".countdoc($ckmonth,$id)."</td>\n";
	...
	echo "</tr>\n";
	$i++;
	$totaldown=$totaldown+countdoc($ckmonth,$id);
...
{
	global $prefix,$db;
	$sqlup="SELECT count(*) AS dem FROM ".$prefix."_document WHERE uadmin=$id AND active=1 AND '$time'=DATE_FORMAT(FROM_UNIXTIME(time),'%Y-%m')";
	//die($sqlup);
	$resultup = $db->sql_query($sqlup);
	if($db->sql_numrows($resultup) > 0) {
	...
	$resultups = $db->sql_query("SELECT u.adacc, u.adname, u.id FROM ".$prefix."_document as d, ".$prefix."_admin as u WHERE d.uadmin=u.id and '$ckmonth'=DATE_FORMAT(FROM_UNIXTIME(d.time),'%Y-%m') AND d.active=1 GROUP BY u.adacc order by u.adacc desc");
		if($db->sql_numrows($resultups) > 0){
			$i=1;
		while(list($fullname, $adname, $id) = $db->sql_fetchrow($resultups)){
	echo "<table  id=\"bang2$i\" border=\"1\" width=\"100%\" cellspacing=\"4\"  cellpadding=\"4\" class=\"tableborder\">\n";
	echo "<tr><td colspan=\"31\" class=\"header\">Bảng 2.$i Thống kê chi tiết thực thu thành viên ".$adname." (".$fullname.")</td></tr>";
	echo "<tr>\n";
	...
	//$totalprice=0;
	$icount=1;
		$sqlup="SELECT title, active, time FROM ".$prefix."_document WHERE uadmin=$id AND active=1 AND '$ckmonth'=DATE_FORMAT(FROM_UNIXTIME(time),'%Y-%m')";
		//die($sqlup);
		$resultup = $db->sql_query($sqlup);
		if($db->sql_numrows($resultup) > 0) {
			while(list($title, $active, $time) = $db->sql_fetchrow($resultup)){
	...
	$icount=$icount-1;
		echo "<tr>\n";
			echo "<td align=\"right\" class=\"row1\" colspan=\"2\"><strong>Tổng tài liệu</strong></td>\n";
			//echo "<td align=\"right\" class=\"row1\"><strong>".bsVndDot($totalprice)."</strong></td>\n";
			echo "<td align=\"right\" class=\"row1\">".$icount."</td>\n";
