...
$result = $db->sql_query("SELECT content FROM ".$prefix."_gentext WHERE textname='scroll' AND alanguage='$currentlang'");
...
if(isset($_POST['submit'])) {
	...
	if($db->sql_numrows($result) > 0) {
		$db->sql_query("UPDATE ".$prefix."_gentext SET content='$content' WHERE textname='scroll' AND alanguage='$currentlang'");
	}else{
		$db->sql_query("INSERT INTO ".$prefix."_gentext (textname, content, alanguage) VALUES ('scroll', '$content', '$currentlang')");
	}
	updateadmlog($admin_ar[0], $adm_modname, _SCROLLTEXT, _SAVECHANGES);
...
echo "<tr><td class=\"header\">"._SCROLLTEXT."</td></tr>";
echo "<tr>\n";
echo "<td>";
