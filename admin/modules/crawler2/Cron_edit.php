...
$query = "SELECT cron_name, cron_url, cat_id, filter_id, alanguage, cdate FROM {$prefix}_ngrab_cron WHERE id=$id";
$result = $db->sql_query($query);
if($db->sql_numrows($result) != 1) header("Location: modules.php?f=$adm_modname&do=cron");
list($cron_name, $cron_url, $cat_id, $filter_id, $alanguage, $cdate) = $db->sql_fetchrow($result);
...
if (isset($_POST['submit'])) {
	...
	if(!$err) {
		$query = $db->sql_query("UPDATE {$prefix}_ngrab_cron SET cron_name='$croname', cron_url='$cronurl', cat_id= $catid, filter_id=$filter, alanguage= '$currentlang' WHERE id=$id");
...
echo "<form action=\"modules.php?f=$adm_modname&do=$do&id=$id\" name=\"adminForm\" method=\"POST\" enctype=\"multipart/form-data\">";
echo "
<div id=\"pagecontent\">
...
echo "<td class=\"row2\"><input type=\"text\" name=\"croname\" value=\"$cron_name\" size=\"60\"></td>\n";
echo "</tr>\n";
echo "<tr>\n";
echo "<td align=\"right\" class=\"row1\"><b>"._SOURCE."</b></td>\n";
echo "<td class=\"row2\"><input type=\"text\" name=\"cronurl\" value=\"$cron_url\" size=\"60\" maxlength=\"253\"></td>\n";
echo "</tr>\n";
...
if($db->sql_numrows($resultfilter) > 0) 
{
	...
	while(list($ft_filter_id, $ft_filter_title) = $db->sql_fetchrow($resultfilter)) 
	{
		if($ft_filter_id == $filter_id){$_selected ="selected";}
		else{$_selected="";}
		echo "<option $_selected value=\"$ft_filter_id\">$ft_filter_title</option>";
	}
	echo "</select></td></tr>";
}
echo "<tr>\n";
...
if($db->sql_numrows($resultcat) > 0) 
{
	...
	$listcat ="";
	while(list($n_cat_id, $n_titlecat) = $db->sql_fetchrow($resultcat)) 
	{
		if($cat_id == $n_cat_id){$_selected ="selected";}
		else{$_selected="";}
		$listcat .= "<option $_selected value=\"$cat_id\">--$n_titlecat</option>";
		$listcat .= subcat($n_cat_id,"-",$cat_id, "");
	}
	echo $listcat;
	echo "</select></td></tr>";
}
