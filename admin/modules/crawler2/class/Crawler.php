...
{	
	...
	public function getPost($_url, array $_replace, $_reg, $_page)
	{
		...
		$_post["images"]		= $this->getImage($_post["content"], $_reg["image"], $_page["url_image"]);
		return $_post;
	}
	...
	public function getImage($_content, $_reg, $_page)
	{
		...
		if(!empty($images))
		{
			...
			foreach($images[2] as $image_link)
			{
				...
				// Replace image url
				$_data =  basename($_dataImage2[$k]);
				$_content	=	str_replace($image_link,$_dataImage2[$k],$_content);
				$k++;
			}
		}
		else
		{
			$_data = "image not fount!";
		}
		return $_data;
	}	
