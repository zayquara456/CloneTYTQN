...
class WebUser extends User {
	...
	function register($url) {
		global $db, $escape_mysql_string, $module_name, $adminmail, $prefix, $activationPeriod, $path_upload;
		...
		if (!empty($this->name) && !empty($this->email) && !empty($this->address) && !empty($this->phone)) {
			...
			$folder = url_optimization(trim($name));
			makeDir("$path_upload/document/$folder");
			$activationCode = md5(uniqid(rand(), true));
			$db->sql_query("SELECT id FROM {$prefix}_user WHERE email='$email'");
			if ($db->sql_numrows() > 0) {
				return WEBUSER_EMAIL_REGISTERED;
			} else {
				$db->sql_query("INSERT INTO {$this->tableName} (title, fullname, email, pass, address, phone, folder, activationCode, registrationTime, loginAttempt) VALUES ({$this->sex}, '$name', '$email', '$pass', '$address', '$phone', '$folder', '$activationCode', NOW(), 0)");
	...
	function getUser($id, $email = '') {
		...
		else $db->sql_query("SELECT id, title, fullname, email, pass, address, phone, loginAttempt FROM {$this->tableName} WHERE fullname='".$escape_mysql_string($email)."' OR email='".$escape_mysql_string($email)."'");
