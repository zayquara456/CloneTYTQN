...
function page_tilecat($catid, $parent, $catname) {
	...
	$titlecat ="";
	$result = $db->sql_query("SELECT catid, parent, title FROM ".$prefix."_news_cat WHERE catid='$parent'");
...
function check_module($parent) {
	global $db, $prefix, $module_name;
	$titlecat ="";
	$result = $db->sql_query("SELECT permalink, parent FROM ".$prefix."_news_cat WHERE catid='$parent'");
	if($db->sql_numrows($result) > 0) {
