...
$w_array=array(280, 191, 191, 162, 162, 162, 162);
$h_array=array(180, 180, 180, 172, 172, 172, 172);
?>
...
<div class="hotdocument">
	<div class="hotdocument-content">
		<div class="fl">
	<?php
	$result = $db->sql_query("SELECT catid, title, images, guid FROM {$prefix}_document_cat WHERE active=1 AND parent=21 AND alanguage='$currentlang' ORDER BY weight LIMIT 7");
...
if($db->sql_numrows($result) > 0) 
{
	...
	?>
		<div class="style<?php echo $i;?>">
			...
			<h2><a href="<?php echo url_sid($s_guid);?>"><?php echo $s_titlecat?> <img src="<?php echo $urlsite;?>/images/w_next_right.png"></a></h><?php echo $i;?>>
		</div>
	<?php
		if($i==2){echo '</div>';}
		$j++;
		...
		while(list($catidcat, $titlecat, $parentcat, $homelinks) = $db->sql_fetchrow($result_catindex)) 
		{
			...
			if($db->sql_numrows($result_newsindex) > 0) 
			{
				...
				while(list($idnewind, $titlenewind,  $imagesind, $timenewind, $price, $fullname, $folder) = $db->sql_fetchrow($result_newsindex)) 
				{
					...
					echo "<div class=\"ui-box-content $paddingright fl\">\n";
					echo "<div class=\"ui-box-img fl\"><div><a href=\"$url_detail\">$imagesind</a>";
					echo '<div class="money-view"><span>'.$price.' EP</span></div>';
					echo "</div></div>\n";
...
elseif($catid==50 || $t=='ban-do-khao-sat')
{
...
<div class="hotdocument">
	<div class="hotdocument-content">
		<div class="fl">
	<?php
	$result = $db->sql_query("SELECT catid, title, images, guid FROM {$prefix}_document_cat WHERE active=1 AND parent=50 AND alanguage='$currentlang' ORDER BY weight LIMIT 7");
		...
		while(list($catidcat, $titlecat, $parentcat, $homelinks) = $db->sql_fetchrow($result_catindex)) 
		{
			...
			if($db->sql_numrows($result_newsindex) > 0) 
			{
				...
				while(list($idnewind, $titlenewind,  $imagesind, $timenewind, $price, $fullname, $folder) = $db->sql_fetchrow($result_newsindex)) 
				{
					...
					echo "<div class=\"ui-box-content $paddingright fl\">\n";
					echo "<div class=\"ui-box-img fl\"><div><a href=\"$url_detail\">$imagesind</a>";
					echo '<div class="money-view"><span>'.check_docdown($idnewind).'</span></div>';
					echo "</div></div>\n";
