...
if(isset($id) && !empty($id) && $module_name =="products") {
	$sql_seld_prdbl = " AND id!='$id'";	
} else {
	$sql_seld_prdbl ="";
}		
...
$result_prd_blnews = $db->sql_query("SELECT id, title, images FROM ".$prefix."_products WHERE active=1 AND ptops=1 AND alanguage='$currentlang' AND images!=''$sql_seld_prdbl ORDER BY time DESC LIMIT $prd_nums_news_bl");
if($db->sql_numrows($result_prd_blnews) > 0) {
	while(list($idprdbln, $titleprdbln, $imgprdbln) = $db->sql_fetchrow($result_prd_blnews)) {
		if($imgprdbln !="" && file_exists("".$path_upload."/products/".$imgprdbln)) {
			if(file_exists("".$path_upload."/products/thumb_".$imgprdbln)) {
				$imgprdbln = "thumb_".$imgprdbln;
			}	
