...
$result = $db->sql_query("SELECT alanguage FROM ".$prefix."_camnang_cat WHERE catid='$catid'");
if(empty($catid) || $db->sql_numrows($result) != 1) {
	header("Location: modules.php?f=".$adm_modname."");
	die();
}	

$db->sql_query("UPDATE ".$prefix."_camnang_cat SET onhome='$stat' WHERE catid='$catid'");
