...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query("INSERT INTO ".$prefix."_mainmenus (mid, parentid, title, url, alanguage, weight, target, menu_type) VALUES (NULL, '$parentid', '$title', '$url', '$currentlang', '$weight', '$newwindow','$menu_type')");
		fixweight_mn();
...
echo "</tr>\n";
$result_cat = $db->sql_query("SELECT mid, title FROM ".$prefix."_mainmenus WHERE $where parentid='0' AND alanguage='$currentlang' ORDER BY weight");
...
echo "<tr><td colspan=\"2\" align=\"center\" class=\"row4\"><input class=\"button2\"  type=\"submit\" name=\"submit\" value=\""._ADD."\"></td></tr>";
echo "</table></form></div>";

//////////////////////////////////DANH SACH MENU//////////////////////////////////////////////////
$perpage = 200;
$offset = ($page-1) * $perpage;
$total = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_mainmenus WHERE $where parentid='0' AND alanguage='$currentlang'"));
$resultcat = $db->sql_query("SELECT mid, title, active, weight, url, target  FROM ".$prefix."_mainmenus WHERE $where parentid='0' AND alanguage='$currentlang' ORDER BY weight,mid ASC LIMIT $offset, $perpage");
...
//} else {
	echo "<td align=\"center\" class=\"row1\"><a href=\"?f=$adm_modname&do=delete&menu_type=$menu_type&mid=$mid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
//}
echo childcat($mid);
}
...
echo "<tr><td class=\"row4\" colspan=\"8\"><select name=\"v\">";
...
function childcat($mid, $text="&nbsp;&nbsp;") {
	...
	if($menu_type!="")
		...
		header("Location: modules.php?f=".$adm_modname."&menu_type=main_menu");
	$treeTemp ="";
	$result = $db->sql_query("SELECT mid, title, active, weight, url, target FROM ".$prefix."_mainmenus WHERE $where parentid='$mid' ORDER BY weight,mid ASC");
	if($db->sql_numrows($result) > 0 ) {
		...
		//	}
			...
			$treeTemp .= "<td align=\"center\" class=\"row3\"><a href=\"?f=$adm_modname&do=delete&menu_type=$menu_type&mid=$mid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
			$treeTemp .= "	</tr>\n";
