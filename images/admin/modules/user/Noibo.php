...
if (!defined('iS_USER') || !isset()) header("Location: ".url_sid("index.php?f=user&do=login")."");
...
$catArr = array();
$cats = $db->sql_query("SELECT catid, title FROM {$prefix}_noibo_cat");
...
$total = $db->sql_numrows($db->sql_query("SELECT id FROM {$prefix}_noibo $where"));
$result = $db->sql_query("SELECT id, catid, title, time, active, hits, nstart FROM {$prefix}_noibo $where $sortby LIMIT $offset, $perpage");
...
<form action="" name="frmtool" method="get">
	<input type="hidden" name="f" value="noibo" />
	<label for="action">Tiêu đề</label>
	<input type="text" id="title" value="" name="title" />
	<label for="cat">Chuyên mục</label>
	<?php
	$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_noibo_cat WHERE parent=0 ORDER BY weight");
